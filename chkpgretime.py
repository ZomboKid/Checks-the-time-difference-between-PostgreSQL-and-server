...
def f_get_time_iside_postgre():
    ...
    time_inside_postgre = datetime.datetime.strptime(result[0].strftime(
                           '%Y-%m-%d %H:%M:%S'), FMT)
...
def f_get_time_on_host():
    time_on_host = datetime.datetime.strptime(datetime.datetime.now(
                    ).strftime('%Y-%m-%d %H:%M:%S'), FMT)
    return time_on_host
# ----------------------------------------------------------------------------
...
def f_chktime():
    ...
    if tdelta.seconds > trigger:
        print >> sys.stderr, "ERROR - timedelta %i sec between Postgre and \
                             host is bigger than max time difference in %i \
                             sec" % (tdelta.seconds, trigger)
        sys.exit(1)
# ----------------------------------------------------------------------------
